...
- **FR-9.** The app SHALL document and support a guaranteed escape path that does not require the app to be responsive:
  ...
  - **FR-9a.** The keyboard-invoked Force Quit dialog (⌥⌘Esc) is **documented as NOT reachable while locked** — it is suppressed by the event tap along with every other keystroke (FR-3, FR-6). The mouse-driven Apple-menu Force Quit (FR-9) is the supported fallback. Rationale: keeping a second always-allowed system chord would dilute FR-6's blanket-suppression contract for negligible safety benefit, given the Apple-menu path is mouse-only and always available.
