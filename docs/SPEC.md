...
2. If `type == .systemDefined` (NX_SYSDEFINED, raw value 14 — not a named `CGEventType`), **drop it** (`return nil`) while locked. **Correction (post-implementation, 2026-05-29):** the v0.3 text said to classify the subtype and pass through non-media systemDefined events. The only public way to read the subtype is `NSEvent(cgEvent:)`, which calls into Text Input Sources (TSM) and **asserts it must run on the main queue** — invoking it from the tap thread crashes the process (`dispatch_assert_queue` → `EXC_BREAKPOINT`/SIGTRAP, observed in the field). We therefore do **not** build an `NSEvent` in the callback and instead drop every systemDefined event. This over-drops the rare non-media systemDefined event during a lock session, which is safe and consistent with suppressing keyboard keys while cleaning. Hard rule: no `NSEvent` / AppKit / TSM call may run on the tap thread.
